...
- **Map the disagreement** (borrowed from `storm`'s contradiction map): name where roles **clash**, where they **converge**, and the **blind spot** — what no role raised that the brief actually needed. The blind spot is often the highest-value output; surface it as an Open Question rather than letting it stay invisible.
...
6. **Confidence vs. grounding** (from `storm`'s peer-review lens): did the panel assert external facts confidently without verifying them? Flag claims that rest on fluency, not evidence. Where the panel was "guessing" about the outside world, recommend a `storm` grounding hand-off to replace assertion with a cited briefing (see Cross-Skill Integration).

**Deliverable:** Structured report with: blind spots found (if any), missing role recommendations, substantive gaps, ungrounded claims flagged for a `storm` hand-off, and overall verdict (ready-to-ship / needs-revision).
...
| `pitch-deck` | When the team needs an investor-ready shippable deck — HTML, Reveal.js, print-to-PDF. Explicit companion to `brand-workshop` + `validation-canvas` + `riskiest-assumption-test` in the startup-artifact chain. Prefer the skill directly for "build me a pitch deck" requests. |
| `startup-grill` | When the team needs an adversarial pre-mortem with a verdict label (Investable / Pivot / Pass). Prefer the skill directly for "grill my startup" / "kill my idea" / "stress-test my pitch" requests. |
| `storm` | When the panel hits a **grounding need** — it's reasoning about external facts it can't verify from memory ("verify the current state of X", "survey what's known about Y", "we're guessing here"). Hand off the question with the panel's active roles as seed perspectives; `storm` runs grounded per-perspective research and returns a cited briefing (every claim sourced or tagged `[unverified]`) that the panel folds back into its conclusion. For a single authoritative answer or a one-claim fact-check, prefer `deep-research` *(if installed — it's an external skill, not bundled here)*; `storm` is multi-perspective, not source-of-truth. If neither retrieval skill is available, fall back to the panel's own retrieval or tell the user which skill to install — never route to a skill that isn't present. |
| [`ui-ux-pro-max`](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) *(if installed)* | When `@senior_product_designer` or `@senior_frontend_engineer` needs to produce high-fidelity, stack-aware UI (50 styles, 21 palettes, 9 stacks). Suggest install if not present and the team is heading into UI execution. |
| [`taste-skill`](https://github.com/Leonxlnx/taste-skill) *(if installed)* | Sibling to `ui-ux-pro-max` for the UI execution layer. Differentiator: three explicit dials — DESIGN_VARIANCE (1–10), MOTION_INTENSITY (1–10), VISUAL_DENSITY (1–10) — that bias generated frontend output away from generic AI defaults ("anti-slop"). Use when `@senior_product_designer` or `@design_engineer` raises "this will look generic" as a real risk, when the user explicitly asks for opinionated layout / motion / density, or when the deliverable is a landing or marketing surface where taste matters more than convention. The two skills stack: `taste-skill` sets the *direction*, `ui-ux-pro-max` produces the stack-specific code. **If not installed and the team would benefit, suggest the user install it** (see the repo README for the `npx` one-liner) before kicking off design execution; do NOT synthesize dial behavior in-context — that defeats the parameterized contract. |
| [`addyosmani/web-quality-skills`](https://github.com/addyosmani/web-quality-skills) *(if installed)* | When the team produces or reviews a web deliverable AND any of: `@senior_frontend_engineer` flags performance / Core Web Vitals / loading concerns; `@accessibility_specialist` is on the team (this entry closes a previously-unaddressed handoff gap for that role); `@senior_product_designer` raises a11y or SEO; the conclusion's Next Steps include Lighthouse audits, CWV fixes, WCAG compliance, or SEO work. Six stack-agnostic skills (React/Vue/Angular/Svelte/Astro/plain HTML): `web-quality-audit`, `performance`, `core-web-vitals`, `accessibility`, `seo`, `best-practices`. Stacks well with `ui-ux-pro-max` / `taste-skill` above — UI direction + code first, then web-quality skills for Lighthouse / WCAG polish. **If not installed and the team is heading into web execution, suggest install** (`/plugin marketplace add addyosmani/web-quality-skills && /plugin install web-quality-skills@addy-web-quality-skills`, or `npx skills add addyosmani/web-quality-skills`) — do NOT synthesize Lighthouse-grade audits in-context; the upstream skills track Lighthouse v13+ migrations we wouldn't keep current. |
| [`GoogleChrome/modern-web-guidance`](https://github.com/GoogleChrome/modern-web-guidance) *(if installed)* | Complement to `addyosmani/web-quality-skills` above — same audience (web execution), different job. Reach for this when the team is **about to author** modern UI behavior that has a native API (View Transitions, Popover, Anchor Positioning, `:user-invalid`, customizable `<select>`, native client AI / translation / summarization, INP optimization, `scheduler.yield`, `fetchLater`, speculation rules, container queries, `oklch`, subgrid) — its eval-calibrated guides bias the agent toward the modern API instead of the legacy JS pattern training data still favors. Use addyosmani for *auditing shipped code*; use this for *guiding new code*. Trigger conditions: `@senior_frontend_engineer` is writing modern UI behavior; `@senior_product_designer` raises a native-API question; user asks "what's the modern way to do X", "is there a native API for this", "avoid the polyfill". **If not installed and the team is heading into modern-web execution, suggest install** (`/plugin marketplace add GoogleChrome/modern-web-guidance && /plugin install modern-web-guidance@googlechrome`, or `npx modern-web-guidance@latest install`) — do NOT synthesize the guides in-context; the upstream skill is eval-calibrated weekly against +29 to +44pp uplift targets that we cannot match. Methodology absorbed (paired gold + negative fixtures, opportunity / uplift framing) into `skill-evaluator` + `ai-eval-review`; the domain content stays upstream where it belongs. |
