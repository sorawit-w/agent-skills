...
| `storm` | When the panel hits a **grounding need** — it's reasoning about external facts it can't verify from memory ("verify the current state of X", "survey what's known about Y", "we're guessing here"). Hand off the question with the panel's active roles as seed perspectives; `storm` runs grounded per-perspective research and returns a cited briefing (every claim sourced or tagged `[unverified]`) that the panel folds back into its conclusion. For a single authoritative answer or a one-claim fact-check, use `deep-research` instead — `storm` is multi-perspective, not source-of-truth. |
