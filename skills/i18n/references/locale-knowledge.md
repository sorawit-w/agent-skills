# Locale & cultural knowledge (shared)

This reference is the shared cultural/linguistic **engine** for two skills:

- **`i18n`** — applies it to produce translation files and localized UI strings.
- **`define`** — applies it to define or translate a word/phrase **inline, in context**.

Both consume the same per-locale **teams** (role tables), **cultural notes**,
**pluralization** categories, and **counter-word** rules below. The only difference is
the surface: `i18n` works on i18n files in a coding project; `define` answers in chat.

Each skill layers its own task-specific roles on top of this shared engine: `i18n` adds
file-handling discipline and its `@ux localization reviewer` / `@i18n engineer` roles;
`define` adds comprehension lenses (lexicographer, etymology, register) for learner-facing
glosses. **Edit locale facts here once — both skills inherit.**
