...
- **Runs an optional generative mascot lane** — artwork-shaped assets (mascot, illustrations) route to an image generator instead of code-SVG: anchor-steered prompt from the brand hexes, seed-locked master, a reproducible `mascot.md` recipe, background cutout, and a character-sheet pair (web + landscape print one-pager) instantiated from bundled templates. Machine-verified end to end by `skills/brand-workshop/scripts/verify.py` (template integrity, palette anchors, Chromium 1-page print gate, cutout report; needs Pillow/numpy/scipy, plus optional playwright + pypdf for the print gate). No generator reachable → ships a first-class prompt brief instead. The favicon and logo never derive from the mascot raster.
