...
- **Reads upstream plugin output** when present. `business-model.md` seeds slides 2, 3, 6, 7 and cross-checks the Ask against the Stress Tests. `brand-kit/design-system.md` supplies visual tokens. `brand-kit/deck/pitch-template.html` provides the structural starting point.
...
- **Business model construction.** If the model isn't crisp, use `business-model-canvas` first. This skill reads the canvas; it doesn't replace the work of producing one.
...
- You just finished `business-model-canvas` and/or `brand-workshop` and want the next artifact: the investor-facing deck.
...
- **Business-model crispness** → `business-model-canvas` (upstream).
...
**Phase 1 — Intake (~5 min).** Read the working directory for `business-model.md`, `brand-kit/design-system.md`, `brand-kit/brand-brief.md`, `brand-kit/deck/pitch-template.html`. Ask the founder which deck variant (cold-email / warm-intro / demo day / follow-up). List missing cardinal slots before launching the interview so the founder can batch context.
...
- **Folder conventions over configuration.** No manifest file in v1. If `brand-kit/` and `business-model.md` are present, read them; if not, fall back gracefully. Write to `pitch/`, never clobber another plugin's output.
...
- **`brand-workshop`** — upstream. Produces the `brand-kit/` this skill reads for visual tokens, and the optional `brand-kit/deck/pitch-template.html` structural reference.
- **`business-model-canvas`** — upstream. Produces `business-model.md`, which this skill reads to seed the Problem, Solution, Business Model, and Traction slides — and to stress-test the Ask against the canvas's Stress Tests section.
...
- **Auto cohort-retention appendix** — when `business-model.md` + a retention data file exist, generate the cohort table as an appendix slide.
