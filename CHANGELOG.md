...
### Changed
- **`sub-agent-delegation.md` § When to Delegate — overhead-check note** — reframes the delegation triggers (">15 min", "3+ files") as signals to *consider* delegation, not mandates, applying the file's own budget-anchor philosophy ("signals, not caps"). The `skill-evaluator` audit confirmed it enables inline work for a small single-threaded task without suppressing delegation on genuinely parallel work.
