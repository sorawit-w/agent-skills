...
### Changed
- **Secret scan (`pre-commit-check.sh`)** now prefers `gitleaks protect --staged` when `gitleaks` is on `PATH` (broader coverage, respects a repo-local `.gitleaks.toml`), falling back to the built-in regex when it's absent. A gitleaks *finding* hard-blocks; a gitleaks *tool error* falls through to the regex rather than phantom-blocking. The block stays non-disablable.
...
- gitleaks is broader than the old regex and will surface more findings; projects with example/test keys should allowlist via `.gitleaks.toml`.
- No new runtime dependency: gitleaks is used only if already on `PATH`; the audit escaping is scriptless.
...
- **Registration** — added to `.claude-plugin/plugin.json` skills array; node + edges in `docs/skill-graph.md` (Code track, stable); README Start-here entry under **Writing & shipping code**.
...
- The verification loop is spike-proven against the MCP and dogfooded end-to-end on a real surface (real axe catch → one-cycle fix → clean at both viewports); gate thresholds and gap-questions will keep tuning with broader mode/surface coverage.
...
### Added
- **Repo-root `CLAUDE.md` → Harness vocabulary → `### Control loop (loop engineering)`** — a subordinate subsection (not a competing table) that names prompt → loop → harness engineering, maps the seven loop primitives to where each is enforced, and explicitly de-duplicates against the existing five harness primitives (loop is the verb, harness is the noun). Documentation only; lives in the authoring context, not `BOOTSTRAP.md`.
- **`coding-rules/resources/workflows/feature.md` § Task Loop — "Cheapen the loop before grinding"** — after two failed fix-test cycles on one task, reduce the cost of a single cycle (minimal reproduction, focused test command, watch mode) before a third attempt. A retroactive trigger that fires on observed signal, where the insight in `debugging.md` was never acted on from the task loop. Cross-references `debugging.md`; does not restate it.
- **`coding-rules/resources/references/sub-agent-delegation.md` — Coordination Rule 6, "Integration gate after fan-out"** — when parallel sub-agents each report done, the coordinator runs one cross-slice verification (full build + union of touched-module tests, run together) before declaring complete. Builds on existing Rule 4 (shared worktree) — no merge step invented. Pointed to from `feature.md` § Validate so it's reachable from the finish path.

### Changed
- **`sub-agent-delegation.md` § When to Delegate — overhead-check note** — reframes the delegation triggers (">15 min", "3+ files") as signals to *consider* delegation, not mandates, applying the file's own budget-anchor philosophy ("signals, not caps"). The two upstream directive sites were relaxed to make the exception actually take effect: `feature.md` §4 now reads "you MUST read the reference *before deciding* — then delegate *unless* its overhead check says inline is cheaper" (was "you MUST … spawn sub-agents instead of implementing everything yourself"), and `sub-agent-delegation.md`'s match line is now "delegate — subject to the overhead check below." A faithful-fixture `skill-evaluator` re-audit (a Codex PR review flagged that the original MUST wording would override the softening — and the first audit's fixture had paraphrased that wording away) confirmed the exception fires inline for a small single-threaded task and still delegates genuinely parallel work.

### Why
A harness whose control loop is unnamed can't be edited deliberately. Naming the loop as the runtime-control-flow half of harness engineering makes the spine legible without a parallel taxonomy — the de-dup line prevents the "which table owns state?" drift the repo's own maintenance note warns about. The two rule edits target real gaps the loop framing exposed: `debugging.md` knew the feedback loop was the speed limit but the task loop never acted on it (cheapen-the-loop), and vertical-slice fan-out reduced conflicts but never verified the *seam* between slices (integration gate). Change 2's trigger ships retroactive rather than predictive because agents reliably fail to predict ">3 cycles" up front. MINOR: additive directives, no breaking change to existing skills.
