...
**Current release: `4.16.0`.** `coding-rules` extends its Conventional Commits convention to **pull requests**: PR titles now follow `<type>[optional scope]: <description>` — explicitly because the skill's recommended squash-merge makes the PR title the squashed commit's subject on the base branch, so a freeform title silently breaks the conventional-commit history. Adds a canonical minimal PR body (`## What & why` + `## How to verify`) and aligns the commit-format notation between `communication.md` and the always-on `BOOTSTRAP.md` (scope is optional, type required). Validated by a split-role `skill-evaluator` audit (16/17 across in-context and discoverability-only conditions). Full version history is in [CHANGELOG.md](CHANGELOG.md).
