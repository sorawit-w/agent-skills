...
**Current release: `4.20.1`.** Doc + authoring-doctrine patch for **`coding-rules`**: fixes the `audit` row rendering detached from the README sub-commands table, and adds a **gate-tiers** table to the rule-editing doctrine (which checks block a merge, tiered by change-risk — and the rule that a clean in-session `skill-evaluator` pass does not authorize merge by itself). The prior **`4.20.0`** added the seventh sub-command, **`audit`** — a read-only static *conformance* audit of a real-coding project against the current rule corpus, emitting a self-contained HTML report under `.ai/audits/`; derived, honest (three-way coverage banner), incremental with aspect scoping, never mutating, with a docs-only **`review` recipe** alongside. Full version history is in [CHANGELOG.md](CHANGELOG.md).
