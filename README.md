...
**Current release: `4.20.0`.** Adds a seventh sub-command to **`coding-rules`**: **`audit`** — a read-only static *conformance* audit of a real-coding project against the current rule corpus, emitting a self-contained HTML report under `.ai/audits/`. It's derived (no hardcoded rule list — it classifies each live rule as auditable/partial/process-only) and honest (a three-way coverage banner names what a static pass can't see; ~7 of ~23 rule areas are checkable). Incremental by default with aspect scoping (`security`/`quality`/`data`/`git-hygiene`/`docs`); never edits, commits, or merges; redirects skill repos to `skill-evaluator`. Plus a docs-only **`review` recipe** for reviewing an already-landed commit against the rules. Full version history is in [CHANGELOG.md](CHANGELOG.md).
