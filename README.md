...
**Current release: `4.19.0`.** Three rule additions to **`coding-rules`**: the agent is now **environment-aware** (a prod-vs-non-prod behavior matrix plus an always-on rule that non-prod must never produce prod-visible side effects and never run prod-affecting ops without confirmation), **redacts secrets in its own chat output** (mask to last-4), and **proposes a materially better approach once** when the user specifies one without planning — then defers, skipping trivial tasks. A split-role `skill-evaluator` audit confirmed all three land and that the propose-once rule no longer contradicts the existing "don't suggest unprompted" guardrail (26/26 assertions). Full version history is in [CHANGELOG.md](CHANGELOG.md).
