...
- [`pixel-art`](#pixel-art) — `whoami` calls it (if an image generator is available) to generate the class character portrait; falls back to a built-in class SVG.
...
`3.13.1` is the current release. Replaces `whoami`'s 13 bundled class portraits with **hi-density pixel art** — a 64×64 logical pixel grid with per-material shade ramps, hue-shifted shadows, a dithered warm-cream background, and crisp 1px silhouette outlines, rendered to 512×512 PNG, up from the first release's coarse ~10-rect SVG emblems. Adds `skills/whoami/assets/characters/generate-portraits.py`, a deterministic Python + Pillow generator that renders the whole set from one parametric figure system (6 axis-family designs — helm / cowl / hood / hat / face — in high/low variants plus Wildcard) so the art is reproducible and restyleable in a single run; `class-map.md`, `SKILL.md`, and the skill README now point at `.png`. Also folds in the batched `skill-evaluator` enhancement — a Step 4 in the findings-report Next-steps flow that capability-gates a `skill-creator` description-check handoff. Additive only — no breaking changes.
