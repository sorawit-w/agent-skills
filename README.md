...
**Current release: `4.13.0`.** `coding-rules` absorbs five surgical ideas from a Claude Code "power phrases" walkthrough, validated against the corpus — most of the video was already covered, so only the deltas that name a recurring failure made it in: blind parallel lenses (same-artifact, different-lens sub-agent review) in `sub-agent-delegation.md`, a tool-to-*see*-output rule in `validation.md`, a Taste-Test (augment-vs-automate) decision filter plus a cost-of-error note on the Reversibility Matrix in `safety-mindset.md`, per-step key-decisions in the plan-approval template, and an interview-pass pointer routing to `team-composer`/`brainstorming`. Declined: skill-authoring ("build me a skill" belongs to `skill-creator`/`skill-evaluator`). Full version history is in [CHANGELOG.md](CHANGELOG.md).
