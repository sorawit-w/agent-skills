...
`3.15.2` is the current release. Two small **`whoami`** consistency fixes surfaced during a README accuracy audit. The "Status and scope" version marker `v0.1.0` → `v0.1`, matching the two-part format the other eleven skills use — the marker is a coarse maturity label, not bumped per release, only the format was off. And the `whoami_version` frontmatter field is removed from `profile-template.md`: a repo-wide grep found it defined once and referenced nowhere — no fill rule, no consumer (`handshake` reads `schema_version`, not this), a latent undefined-state placeholder. `generated` + `source` already stamp provenance; `schema_version` (unchanged at 2) covers format detection. PATCH — doc/template cleanup, consumer-invisible, no behavior change.

Earlier in v3.15.1: Adds **absent-state test coverage** to `skill-evaluator` — the gap that let a silent-data-loss bug pass a clean 33/33 audit in v3.14.0. Phase 3 gains a fifth test category, Absent-state tests: for every resource a rule quietly assumes exists (an upstream artifact, a canonical file, a populated field), a test where that resource is absent — with the load-bearing rule that the fixture must actually withhold it, since a fixture that supplies it hides the bug. The harness lens gains a sixth question, Undefined-state coverage, naming the same gap at the shape level. A classification carve-out in Phase 5 + `fix-taxonomy.md` routes an absent-state failure to Layer 1 (skill text), not Layer 4 (fixture) — without it the failure mis-routes to "add the fixture," deleting the test. PATCH — additive audit guidance, no workflow-logic or contract change; precedent v3.6.3 shipped the harness lens itself as a doc patch.

Earlier in v3.15.0: Rewrites **`whoami`**'s summary-authoring house style. The character-sheet summary is now written as the agent speaking to the user — second person ("You think in routes..."), one warm conversational voice that reads the user and states the agent's own side of the deal — instead of the old structure-first dial readout. Four rules replace the prior guidance: interpret the dials but never recite them (no "(Breadth 9)" parentheticals), conversational prose with no bold anchors, about three short paragraphs on a loose arc rather than a rigid one-facet-per-paragraph template, and no bio recitation — let the characterization carry who the person is. The structure-first guidance produced summaries that read like a parameter dump; the summary's job is to interpret the six dials into a human read, not echo numbers the sheet already shows. Voice chosen from a six-style exploration. `references/persistence.md`-only change — reviewed in-context, no SKILL.md text touched. MINOR — no trigger or contract change; `handshake` reads frontmatter, not the summary.

Earlier in v3.14.1: Hardens **`whoami`**'s Regenerate flow against a silent-data-loss defect. Regenerate now renders only from the canonical `whoami-profile.md`; when that file is missing it **stops and renders nothing** — no fallback to the prose memory entry or a snapshot — and directs the user to `/whoami` or `/whoami rerun` to build the profile. The defect: a regeneration with no canonical profile present silently produced a character sheet with an empty Specializations section, because `flexible_traits` lived only in the prose memory entry, which carries no structured array for the HTML template. `persistence.md` already named this failure class ("drops data the profile holds") as the bug to guard against — the Regenerate flow simply had no defined behavior when its source of truth was absent, so it improvised. Fail-loud was chosen over self-heal: reconstructing the profile from prose is the same fragility that caused the bug. PATCH — fix-only, no trigger or contract change.

Earlier in v3.14.0: Refines **`whoami`** in response to an external 6/10 review and adds a social-share clarity fix to the character sheet. The interview gains an **anti-patterns question** — it now asks what AI assistants most often get wrong and captures 2–3 named failure modes into the portable profile, the runtime memory entry, and the HTML sheet; profile `schema_version` bumps 1 → 2 with an additive `anti_patterns` field (`handshake`'s read contract is unaffected). Core Principle 4 is rewritten from passive ("hold the profile lightly") to **active drift-handling** — when live behavior contradicts a stored dial, follow the live behavior, flag the mismatch, and revise on confirmation. Out-of-scope routing is made explicit in `SKILL.md` and the README: per-task / per-mode calibration and the code definition-of-done go to `handshake` / `coding-rules`, correction-accrual to `feedback`-type memory; the per-mode-profile and chat-mining non-goals are recorded as deliberately declined. The character sheet gains a **header subtitle** ("An AI collaboration profile") and a **footer legend** ("Dials are collaboration preferences, not scores") so a sheet shared on social media isn't misread as a competence scorecard. Pre-shipment `skill-evaluator` audit: 33/33 assertions across 6 tests. MINOR — additive only, no breaking changes.
